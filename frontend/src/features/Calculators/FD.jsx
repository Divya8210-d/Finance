...
export default function FD({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/v1/fd", Data, {
        withCredentials: true,
      });
      const result = res.data?.data?.maturedAmount;

      if (result !== undefined) {
        setMaturedAmount(result);
      } else {
        setMaturedAmount("Unable to calculate. Invalid response.");
      }
    } catch (err) {
      const message = err.response?.data?.message || "An unknown error occurred";
      console.log("Calculating error", message);
      setMaturedAmount(message);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/30">
      <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md dark:bg-gray-800">
        <h2 className="text-xl font-bold mb-4 text-center">Fixed Deposit Calculator</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Compounded</label>
            <input
              type="text"
              value={compounded}
              required
              onChange={(e) => setCompounded(e.target.value)}
              className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-blue-400 dark:bg-gray-700"
              placeholder="How is it compounded? (e.g., monthly, yearly)"
            />
          </div>
        ...
        {maturedAmount && (
          <div className="mt-6 p-4 bg-green-100 text-green-800 rounded-lg shadow-sm text-center dark:bg-green-800 dark:text-white">
            <strong>Matured Amount:</strong> ₹{maturedAmount}
          </div>
        )}
      </div>
    </div>
